<template>
  <div class="bg-gray-50 shadow-sm rounded-lg p-6">
    <div class="text-center">
      ...
      <h2 class="text-2xl font-medium mb-2">{{ t("checkout.confirmation.thank_you") }}</h2>
      <p class="text-gray-600 mb-6">{{ t("checkout.confirmation.order_number") }}: #{{ orderId }}</p>
      
      <div class="text-left max-w-2xl mx-auto space-y-4">
        <div>
          <h3 class="font-medium mb-2">{{ t("checkout.confirmation.order_details") }}</h3>
          <p class="text-gray-600">{{ t("checkout.confirmation.order_date") }}: {{ orderDate }}</p>
          <p class="text-gray-600">{{ t("checkout.confirmation.estimated_delivery") }}: {{ estimatedDelivery }}</p>
        </div>
