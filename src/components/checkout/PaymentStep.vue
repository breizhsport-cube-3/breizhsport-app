...
const handleChange = (event) => {
  ...
  if (event.error) {
    cardErrorsRef.value.textContent = t(`checkout.payment.errors.${event.error.code}`) || event.error.message;
  } else {
...
const handlePayment = async () => {
  ...
  try {
    ...
    if (error) {
      cardErrorsRef.value.textContent = t(`checkout.payment.errors.${error.code}`) || error.message;
      emit('valid', false);
      return;
    }
    emit('valid', true);
    ...
    cardErrorsRef.value.textContent = t('checkout.payment.errors.processing_error');
    emit('valid', false);
  } finally {
