<template>
    <div class="space-y-6">
      ...
      <div class="flex gap-4 overflow-x-auto pb-4">
        <div v-for="(carrier, id) in shippingData.carriers" 
             :key="id"
          ...
          <img :src="carrierLogos[id]" :alt="carrier.name" class="h-12 object-contain mb-2" />
      ...
      <div v-if="selectedCarrier" class="space-y-4">
        <div v-for="method in currentCarrierMethods" 
             :key="method.id"
             @click="selectMethod(method)"
             class="p-4 border rounded-lg cursor-pointer"
             :class="{ 'border-primary-600 bg-primary-50': isMethodSelected(method) }">
          ...
          <div class="flex flex-wrap gap-3 mt-3">
            <span v-for="feature in method.features" 
                  :key="feature"
  ...
  const formatDeliveryTime = (estimatedDays) => {
    if (estimatedDays.min === estimatedDays.max) {
      return t('checkout.shipping.delivery_time_single', { days: estimatedDays.min });
    }
